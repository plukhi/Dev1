...
(function(){
    ...
    angular.module('gantt').factory('GanttObjectModel', ['ganttUtils', 'moment', function(utils, moment) {
        ...
        ObjectModel.prototype.cleanTask = function(model) {
            ...
            if (model.fromActual !== undefined && !moment.isMoment(model.fromActual)) {
                model.fromActual = moment(model.fromActual);
            }

            if (model.toActual !== undefined && !moment.isMoment(model.toActual)) {
                model.toActual = moment(model.toActual);
            }
...
(function(){
    ...
    angular.module('gantt').factory('GanttRow', ['GanttTask', 'moment', '$filter', function(Task, moment, $filter) {
        ...
        Row.prototype.addTask = function(taskModel, viewOnly) {
            ...
            var task,actTask, isUpdate = false;
            ...
            if (taskModel.id in this.tasksMap) {
				...
				actTask = new Task(this, taskModel);
        ...
        Row.prototype.moveTaskToRow = function(task, viewOnly) {
			...
			task.model.ac_id=task.row.model.ac_id;
		...
		Row.prototype.setFromToByActualTask = function(task) {
            this.setFromToByValues(task.model.fromActual, task.model.toActual);
        };
    ...
    angular.module('gantt').factory('GanttTask', ['moment', function(moment) {
        ...
        Task.prototype.getBackgroundElement = function() {
            if (this.$element !== undefined) {
				//alert('update getBackgroundElement');
                var backgroundElement = this.$element[0].querySelector('.gantt-task-background');
				//alert('backgroundElement '+backgroundElement);
        ...
        Task.prototype.getContentElement = function() {
            if (this.$element !== undefined) {
				//	alert('update getContentElement');
        ...
        Task.prototype.setTo = function(x, magnetEnabled) {
            ...
            this.updatePosAndSize();
		     ...
		     // Expands the start of the task to the specified position (in em)
        Task.prototype.setFromActual = function(x, magnetEnabled) {
            this.model.fromActual = this.rowsManager.gantt.getDateByPosition(x, magnetEnabled);
            this.row.setFromTo();
            this.updatePosAndSize();
        };

        // Expands the end of the task to the specified position (in em)
        Task.prototype.setToActual = function(x, magnetEnabled) {
            this.model.toActual = this.rowsManager.gantt.getDateByPosition(x, magnetEnabled);
            this.row.setFromTo();
            this.updatePosAndSize();
        };
...
angular.module('gantt.templates', []).run(['$templateCache', function($templateCache) {
    $templateCache.put('template/gantt.tmpl.html',
        ...
        '        <div class="gantt-task-background" ng-style="{\'background-color\': task.model.mainColor}"></div>\n' +
        '    </script>\n' +
        '\n' +
