                ...
                api.directives.on.new(scope, function(directiveName, directiveScope, element) {
                    if (directiveName === 'ganttRow') {
                        var addNewTask = function(x) {
                            ...
                            var taskActual = directiveScope.row.addTask(taskModel);
        ...
        function(mouseButton, mouseOffset, smartEvent, movableOptions, utils, dom, $window, $document, $timeout) {
            ...
            return {
                ...
                link: function(scope, element, attrs, ganttCtrl) {
                    ...
                    var moveStartX;
					var sourceRowObj,targetRowObj;
						var originalSourceRow ;
                    api.directives.on.new(scope, function(directiveName, taskScope, taskElement) {
                        ...
                        if (directiveName === 'ganttTask') {
							...
							var moveTask= function(evt) {
                                ...
                                if (taskScope.task.moveMode === 'M') {
									...
									console.log('flow 2 befor task.moveto');
                            ...
                            var moveTaskCust = function(evt) {
								
                                var oldTaskHasBeenChanged = taskHasBeenChanged;
				
                                var mousePos = mouseOffset.getOffsetForElement(ganttBodyElement[0], evt);
                                var x = mousePos.x;
								var OriginalX= mousePos.x;
								var OriginalY= mousePos.y;
                                ...
                                if (taskScope.task.moveMode === 'M') {
                                    ...
                                    if (allowRowSwitching) {
                                        ...
                                        var sourceRow = taskScope.task.row;
										 originalSourceRow = taskScope.task.row;
                            ...
                            var disableMoveMode = function() {
                                ...
                                if (taskHasBeenChanged === true) {
									console.log('taskHasBeenChanged '+taskHasBeenChanged);
									...
									taskScope.task.moveMode = undefined;
									 ...
									 taskScope.task.model.from=taskScope.task.originalModel.from;
									 taskScope.task.model.to=taskScope.task.originalModel.to;
                                     taskScope.task.model.lct= taskScope.task.originalModel.lct;
                                     taskScope.task.model.est=taskScope.task.originalModel.est 
									 taskScope.task.originalModel= taskScope.task.model;
                                    if (taskScope.task.row.model.id !== taskScope.task.originalRow.model.id) {
                                        var targetRow = taskScope.task.row;
                                        targetRow.removeTask(taskScope.task.model.id, false, true);
            ...
            link: function(scope, element, attrs, ganttCtrl) {
                ...
                if (scope.content === undefined) {
                    scope.content = '{{task.model.name}}</br>'+
                                    '<small>'+
                                    'Planned : {{task.isMilestone() === true && getFromLabel() || getFromLabel() + \' - \' + getToLabel()}}'+
                                    '</small></br>'+'<small>'+
                                    'Actual : {{task.isMilestone() === true && getFromActualLabel() || getFromActualLabel() + \' - \' + getToActualLabel()}}'+
                                    '</small></br>';
                }
...
(function(){
    'use strict';
    angular.module('gantt.contexts', ['gantt', 'gantt.contexts.templates']).directive('ganttContexts', ['$compile', '$document', function($compile, $document) {
        return {
            restrict: 'E',
            require: '^gantt',
            scope: {
            ...
            link: function(scope, element, attrs, ganttCtrl) {
                ...
                // Load options from global options attribute.
                if (scope.options && typeof(scope.options.context) === 'object') {
                    for (var option in scope.options.context) {
                        scope[option] = scope.options[option];
                    }
                }
                ...
                api.directives.on.new(scope, function(directiveName, taskScope, taskElement) {
                    if (directiveName === 'ganttTask') {
						      var contextScope = taskScope.$new();

							contextScope.pluginScope = scope;
							taskElement.on('contextmenu', function(event){
								  event.preventDefault();
								   //var ifElement = $document[0].createElement('div');
								//   var contextElement = $document[0].createElement('gantt-context');
							//contextElement.style.cssText='position:absolute;top:300px;left:300px;';
									 //  angular.element(ifElement).append(contextElement);
									//taskElement.append($compile(ifElement)(contextScope));
									
									var newDirective = angular.element('<div\n' +
										'>' +
											' <div class="list-group">'+
											   ' <a href="#" id="1" name="delete" class="list-group-item">Delete Task</a>'+
												'<a href="#" id="2" name="detail" class="list-group-item">View Detail</a>'+
											   ' <a href="#" id="3" name="changepln"class="list-group-item">Change Planned</a>'+
											     ' <a href="#" id="4" name="changeact"class="list-group-item">Change Actual</a>'+
											' </div>'+
									   '</div>\n');
									...
									newDirective.on('mouseleave', function(){
											   newDirective.css('display','none');
											});
											var options = newDirective.children();
									newDirective.on('click', function(evt){
										
											    var isFirstChild = (evt.target.parentElement.children[0] === evt.target);
												var contextAction=evt.target.name;
												if(contextAction.trim().toLowerCase()=='delete'){
													 if(taskScope.row.removeTask(taskScope.task.model.id)){
															taskScope.row.$scope.$digest();
														  }
												}else if(contextAction.trim().toLowerCase()=='detail'){
														alert('View More Detail ');
												}else if(contextAction.trim().toLowerCase()=='changepln'){
														alert('Changing Planned time ');
												}else if(contextAction.trim().toLowerCase()=='changeact'){
														alert('Changing Act time ');
												}
            ...
            controller: ['$scope', '$element', function($scope, $element) {
                ...
                $scope.$watchGroup(['simplifyMoment(task.model.est)', 'simplifyMoment(task.model.lct)', 'task.left', 'task.width'], function() {
                    ...
                    if ($scope.task.model.est === undefined || $scope.task.model.lct === undefined) {
					 ...
					 $element.css('background-color',$scope.task.model.actColor);
                });
    ...
    angular.module('gantt.tooltips').directive('ganttTooltip', ['$log','$timeout', '$compile', '$document', '$templateCache', 'ganttDebounce', 'ganttSmartEvent', function($log, $timeout, $compile, $document, $templateCache, debounce, smartEvent) {
        ...
        return {
            ...
            controller: ['$scope', '$element', 'ganttUtils', function($scope, $element, utils) {
				...
				var displayContext = function(newValue, showDelayed) {
                
                 

                    
                    if ( mouseEnterX !== undefined && newValue) {
                        if (showDelayed) {
                ...
                $scope.getToLabel = function() {
				 ...
				 $scope.getFromActualLabel = function() {
                    var taskTooltips = $scope.task.model.tooltips;
                    var rowTooltips = $scope.task.row.model.tooltips;
					...
					if ($scope.task.model.est != undefined) {
                         var dateFormat = utils.firstProperty([taskTooltips, rowTooltips], 'dateFormat', $scope.pluginScope.dateFormat);
						return $scope.task.model.est.format(dateFormat);
					}
                    }

                   

                $scope.getToActualLabel = function() {
                    var taskTooltips = $scope.task.model.tooltips;
                    var rowTooltips = $scope.task.row.model.tooltips;
					...
					if ($scope.task.model.lct != undefined) {
                              var dateFormat = utils.firstProperty([taskTooltips, rowTooltips], 'dateFormat', $scope.pluginScope.dateFormat);
							return $scope.task.model.lct.format(dateFormat);
					}
...
(function() {
    'use strict';
    angular.module('gantt.contexts').directive('ganttContext', ['$log','$timeout', '$compile', '$document', '$templateCache', 'ganttDebounce', 'ganttSmartEvent', function($log, $timeout, $compile, $document, $templateCache, debounce, smartEvent) {
        // This tooltip displays more information about a task
        ...
        return {
            ...
            templateUrl: function(tElement, tAttrs) {
                var templateUrl;
                if (tAttrs.templateUrl === undefined) {
                    templateUrl = 'plugins/tooltips/context.tmpl.html';
                } else {
                    templateUrl = tAttrs.templateUrl;
                }
                if (tAttrs.template !== undefined) {
            ...
            controller: ['$scope', '$element', 'ganttUtils', function($scope, $element, utils) {
                ...
                var mouseMoveHandler;
                  ...
                  $scope.task.$element.bind('contextmenu', function(evt) {
                    mouseEnterX = evt.clientX;
					mouseEnterY = evt.clientY;
				
					$scope.taskRect = parentElement[0].getBoundingClientRect();
                   //showContextAtx(mouseEnterX,mouseEnterY);
					// $element.toggleClass('ng-hide', false);
                });
				   ...
				   /*var showContext = function(newValue, showDelayed) {
                
                  
                    if ( !visible && mouseEnterX !== undefined && newValue) {
                   
                            showContextAtx(mouseEnterX);
				...
				var showContextAtx = function(x,y) {
                    visible = true;
                    

                    $scope.displayedContext = true;
                ...
                var hideContextAtx = function() {
                    visible = false;
              
                    $scope.$evalAsync(function() {
                        $scope.displayedContext = false;
                    });
                };

                var updateContext = function(x,y) {
                    // Check if info is overlapping with view port
					//alert('x '+x);
					//alert('y '+y);
                    /*if (x + $element[0].offsetWidth > getViewPortWidth()) {
                        $element.css('left', (x + 20 - $element[0].offsetWidth) + 'px');
						 $element.css('top', (y + 20 - $element[0].offsetWidth) + 'px');
                        $scope.isRightAligned = true;
                    } else {
                        $element.css('left', (x - 20) + 'px');
						  $element.css('top', (y - 20) + 'px');
                        $scope.isRightAligned = false;
                    }*/
					//alert('hi3');
					     $element.css('left', (x + 20) + 'px');
...
angular.module('gantt.contexts.templates', []).run(['$templateCache', function($templateCache) {
    $templateCache.put('plugins/tooltips/context.tmpl.html',
         '<div\n' +
		 'ng-show="true"'+
		 'ng-style="{top: taskRect.top + \'px\', marginTop: -elementHeight - 8 + \'px\'}"\n' +
        '     class="gantt-context-info">' +
			' <div class="list-group">'+
